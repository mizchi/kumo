...
func (s *Service) CreateWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeAPSError(w, "InvalidRequest", "failed to read body", http.StatusBadRequest)
	...
	if len(body) > 0 {
		if err := json.Unmarshal(body, &req); err != nil {
			writeAPSError(w, "InvalidRequest", "malformed JSON", http.StatusBadRequest)
...
func (s *Service) proxy(w http.ResponseWriter, r *http.Request, backendPath string) {
	...
	if s.backend == "" {
		writeAPSError(w, "BackendUnavailable",
	...
	if s.backendErr != nil {
		writeAPSError(w, "BackendInvalid", s.backendErr.Error(), http.StatusInternalServerError)
...
func handleStorageError(w http.ResponseWriter, err error) {
	...
	if errors.As(err, &ampErr) {
		...
		writeAPSError(w, ampErr.Code, ampErr.Message, status)

		return
	}

	writeAPSError(w, "InternalError", err.Error(), http.StatusInternalServerError)
}

// writeAPSError writes the AWS-shape error body.
func writeAPSError(w http.ResponseWriter, code, message string, status int) {
